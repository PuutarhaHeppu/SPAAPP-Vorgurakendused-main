  ...
  <div
    ...
    <div v-if="showPopup" class="popup">
      <div class="popup-inner">
        ...
        <ul>
          <li v-for="(value, key) in selectedPeople" :key="key">
            {{ key }}: {{ value }}
          </li>
        </ul>
...
const PeoplePeople = usePeopleStore();
const { People } = storeToRefs(PeoplePeople);
...
const remove = (People: People) => {
  PeoplePeople.deletePeople(People);
};
...
.popupClose {
  ...
  padding: 0; /* Remove padding to prPeople misalignment */
}
